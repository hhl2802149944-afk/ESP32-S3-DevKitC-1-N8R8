...
void init_camera_uart() {
    ...
    uart_driver_install(CAM_UART_NUM, 1024, 0, 0, NULL, 0);
    uart_param_config(CAM_UART_NUM, &uart_config);
    uart_set_pin(CAM_UART_NUM, CAM_TX, CAM_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
}

// 发送摄像头指令 (示例：很多玩具摄像头通过特定 Hex 控制)
void send_camera_cmd(const uint8_t* data, size_t len) {
...
void setup() {
    ...
    // 初始化摄像头串口 (针对 T, R 接口)
    init_camera_uart();
    ESP_LOGI(TAG, "Camera UART initialized on TX:%d RX:%d", CAM_TX, CAM_RX);
