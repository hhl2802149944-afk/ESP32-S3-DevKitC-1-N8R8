...
void motor_task(void *pvParameters) {
    while(1) {
        // --- 驱动器 1 (BTS7960) ---
        if (motor1_state == M_FORWARD) {
            gpio_set_level((gpio_num_t)MOTOR1_RPWM, 1);
            gpio_set_level((gpio_num_t)MOTOR1_LPWM, 0);
        } else if (motor1_state == M_BACKWARD) {
            gpio_set_level((gpio_num_t)MOTOR1_RPWM, 0);
            gpio_set_level((gpio_num_t)MOTOR1_LPWM, 1);
        } else {
            gpio_set_level((gpio_num_t)MOTOR1_RPWM, 0);
            gpio_set_level((gpio_num_t)MOTOR1_LPWM, 0);
        }
        ...
        // --- 驱动器 2 (BTS7960) ---
        if (motor2_state == M_FORWARD) {
            gpio_set_level((gpio_num_t)MOTOR2_RPWM, 1);
            gpio_set_level((gpio_num_t)MOTOR2_LPWM, 0);
        } else if (motor2_state == M_BACKWARD) {
            gpio_set_level((gpio_num_t)MOTOR2_RPWM, 0);
            gpio_set_level((gpio_num_t)MOTOR2_LPWM, 1);
        } else {
            gpio_set_level((gpio_num_t)MOTOR2_RPWM, 0);
            gpio_set_level((gpio_num_t)MOTOR2_LPWM, 0);
        }
...
void setup() {
    ...
    motor_conf.pin_bit_mask = (1ULL << MOTOR1_RPWM) | (1ULL << MOTOR1_LPWM) |
                              (1ULL << MOTOR2_RPWM) | (1ULL << MOTOR2_LPWM);
