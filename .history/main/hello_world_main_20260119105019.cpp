...
extern "C" void app_main(void) {
    ...
    // 3. 运行 Arduino 的 setup
    setup();

    // 4. 创建一个单独的任务来运行 Arduino 的 loop
    xTaskCreate([](void*){
        while(1) {
            loop();
            // 注意：在 ESP32 上 loop 中不要用死循环，需要给系统让出时间
            vTaskDelay(1); 
        }
    }, "loop_task", 4096, NULL, 1, NULL);
}
