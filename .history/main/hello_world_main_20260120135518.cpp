...
#define MOTOR1_BIN2     15
#define MOTOR1_PWMB     16
#define MOTOR1_STBY     17

// 电机 2 引脚 (再次优化引脚，避开 47)
#define MOTOR2_AIN1     1
#define MOTOR2_AIN2     2
#define MOTOR2_PWMA     3
#define MOTOR2_BIN1     8
#define MOTOR2_BIN2     18
#define MOTOR2_PWMB     47
#define MOTOR2_STBY     21
...
static int motor_step_delay = 100; // 降低到 100ms，方便排查震荡问题
...
void motor_task(void *pvParameters) {
    ...
    int counter = 0;
    while(1) {
        // 每隔 20 次循环（约 1 秒）打印一次状态，防止刷屏
        if (++counter >= 20) {
            ESP_LOGI(TAG, "Motor Status: M1=%d (step %d), M2=%d (step %d)", motor1_state, step1, motor2_state, step2);
            counter = 0;
        }
        ...
        if (motor1_state != M_STOP) {
            ...
            switch(abs(step1) % 4) {
                case 0: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 1); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR1_BIN1, 1); gpio_set_level((gpio_num_t)MOTOR1_BIN2, 0); break;
                case 1: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 1); 
                        gpio_set_level((gpio_num_t)MOTOR1_BIN1, 1); gpio_set_level((gpio_num_t)MOTOR1_BIN2, 0); break;
                case 2: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 1); 
                        gpio_set_level((gpio_num_t)MOTOR1_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_BIN2, 1); break;
                case 3: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 1); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR1_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_BIN2, 1); break;
            }
...
// 电机 2: 单相通电 (Wave Drive) - 适合 5V 供电，防止电流过大导致的震荡
        if (motor2_state != M_STOP) {
            gpio_set_level((gpio_num_t)MOTOR2_STBY, 1);
            switch(abs(step2) % 4) {
                case 0: gpio_set_level((gpio_num_t)MOTOR2_AIN1, 1); gpio_set_level((gpio_num_t)MOTOR2_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR2_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_BIN2, 0); break;
                case 1: gpio_set_level((gpio_num_t)MOTOR2_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR2_BIN1, 1); gpio_set_level((gpio_num_t)MOTOR2_BIN2, 0); break;
                case 2: gpio_set_level((gpio_num_t)MOTOR2_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_AIN2, 1); 
                        gpio_set_level((gpio_num_t)MOTOR2_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_BIN2, 0); break;
                case 3: gpio_set_level((gpio_num_t)MOTOR2_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR2_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_BIN2, 1); break;
            }
...
void setup() {
    ...
    // 强制重置所有电机引脚，防止被之前的 JTAG 或寄存器残留干扰
    int motor_pins[] = { MOTOR1_AIN1, MOTOR1_AIN2, MOTOR1_PWMA, MOTOR1_BIN1, MOTOR1_BIN2, MOTOR1_PWMB, MOTOR1_STBY,
                         MOTOR2_AIN1, MOTOR2_AIN2, MOTOR2_PWMA, MOTOR2_BIN1, MOTOR2_BIN2, MOTOR2_PWMB, MOTOR2_STBY };
    for(int i=0; i<14; i++) {
        gpio_reset_pin((gpio_num_t)motor_pins[i]);
        gpio_set_drive_capability((gpio_num_t)motor_pins[i], GPIO_DRIVE_CAP_3); // 增强驱动能力
    }
