...
static MotorState motor_state = M_STOP;

// 步进电机脉冲任务
void motor_task(void *pvParameters) {
    int step = 0;
    while(1) {
        if (motor_state != M_STOP) {
            switch(abs(step) % 4) {
                case 0: gpio_set_level((gpio_num_t)MOTOR_AIN1, 1); gpio_set_level((gpio_num_t)MOTOR_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR_BIN2, 0); break;
                case 1: gpio_set_level((gpio_num_t)MOTOR_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR_BIN1, 1); gpio_set_level((gpio_num_t)MOTOR_BIN2, 0); break;
                case 2: gpio_set_level((gpio_num_t)MOTOR_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR_AIN2, 1); 
                        gpio_set_level((gpio_num_t)MOTOR_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR_BIN2, 0); break;
                case 3: gpio_set_level((gpio_num_t)MOTOR_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR_BIN2, 1); break;
            }
            if (motor_state == M_FORWARD) step++; else step--;
            vTaskDelay(pdMS_TO_TICKS(10)); // 转速控制 (10ms)
        } else {
            gpio_set_level((gpio_num_t)MOTOR_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR_AIN2, 0);
            gpio_set_level((gpio_num_t)MOTOR_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR_BIN2, 0);
