...
// 电机 2 引脚 (最终安全方案 - 避开 9-14, 18-19)
#define MOTOR2_AIN1     1
#define MOTOR2_AIN2     2
#define MOTOR2_PWMA     42
#define MOTOR2_BIN1     40
#define MOTOR2_BIN2     41
#define MOTOR2_PWMB     39
    ...
    while(1) {
        ...
        if (motor2_state != M_STOP) {
            switch(abs(step2) % 4) {
                ...
                case 1: gpio_set_level((gpio_num_t)MOTOR2_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_AIN2, 1); 
                        gpio_set_level((gpio_num_t)MOTOR2_BIN1, 1); gpio_set_level((gpio_num_t)MOTOR2_BIN2, 0); break;
                ...
                case 3: gpio_set_level((gpio_num_t)MOTOR2_AIN1, 1); gpio_set_level((gpio_num_t)MOTOR2_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR2_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR2_BIN2, 1); break;
            }
        ...
        vTaskDelay(pdMS_TO_TICKS(20)); // 步进延迟 20ms
    }
}
...
esp_err_t api_motor_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
        ...
        if (motor_id == 2) {
            ...
            ESP_LOGI(TAG, "Motor 2 Cmd -> %s (state=%d)", dir, next_state);
        } else {
            motor1_state = next_state;
            ESP_LOGI(TAG, "Motor 1 Cmd -> %s (state=%d)", dir, next_state);
        }
    }
    httpd_resp_set_type(req, "application/json");
