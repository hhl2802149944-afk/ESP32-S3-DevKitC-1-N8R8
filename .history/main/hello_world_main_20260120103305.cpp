...
// 电机 2 引脚 (TB6612 2)
#define MOTOR2_AIN1     8
#define MOTOR2_AIN2     18
#define MOTOR2_BIN1     19
#define MOTOR2_BIN2     20
// STBY, PWMA, PWMB 可以共用 6, 16, 17 节省排线
        ...
        if (motor1_state != M_STOP) {
            switch(abs(step1) % 4) {
                case 0: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 1); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 0); 
                        gpio_set_level((gpio_num_t)MOTOR1_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_BIN2, 0); break;
                case 1: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 0); 
                ...
                case 2: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 1); 
                        gpio_set_level((gpio_num_t)MOTOR1_BIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_BIN2, 0); break;
                case 3: gpio_set_level((gpio_num_t)MOTOR1_AIN1, 0); gpio_set_level((gpio_num_t)MOTOR1_AIN2, 0); 
...
esp_err_t api_motor_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
        ...
        if (strcmp(item, "2") == 0) motor2_state = next_state;
        else motor1_state = next_state;

        ESP_LOGI(TAG, "Motor %s Cmd -> %s", item, dir);
    }
    httpd_resp_set_type(req, "application/json");
...
void setup() {
    ...
    motor_conf.pin_bit_mask = (1ULL << MOTOR_AIN1) | (1ULL << MOTOR_AIN2) | (1ULL << MOTOR_PWMA) |
                              (1ULL << MOTOR_BIN1) | (1ULL << MOTOR_BIN2) | (1ULL << MOTOR_PWMB) |
                              (1ULL << MOTOR_STBY);
    ...
    // 默认开启使能
    gpio_set_level((gpio_num_t)MOTOR_STBY, 1);
    gpio_set_level((gpio_num_t)MOTOR_PWMA, 1); // 暂时给高电平，之后可用 PWM 调速
    gpio_set_level((gpio_num_t)MOTOR_PWMB, 1);

    // 启动电机控制任务 (分配在 Core 1 以免影响 Wi-Fi)
