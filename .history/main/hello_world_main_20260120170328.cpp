...
// 电机 2 引脚 (避免使用 GPIO 11, 12, 13, 因为它们在 S3 上可能与 Flash/VDD_SPI 冲突)
#define MOTOR2_AIN1     14
#define MOTOR2_AIN2     15
#define MOTOR2_PWMA     16
#define MOTOR2_BIN1     17
#define MOTOR2_BIN2     18
#define MOTOR2_PWMB     39
#define MOTOR2_STBY     40
...
void motor_task(void *pvParameters) {
    ...
    while(1) {
        ...
        if (motor1_state != M_STOP) {
            ...
            int s = (step1 % 4 + 4) % 4; // 处理负数，确保相序循环正确
        ...
        if (motor2_state != M_STOP) {
            ...
            int s = (step2 % 4 + 4) % 4; // 处理负数，确保相序循环正确
