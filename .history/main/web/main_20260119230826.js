document.addEventListener('DOMContentLoaded', () => {
    ...
    // 电机控制逻辑
    const motorAction = async (dir) => {
        msgBar.innerText = `正在发送指令: ${dir}...`;
        try {
            await fetch(`/api/motor?dir=${dir}`);
            msgBar.innerText = `执行成功: ${dir}`;
    ...
    document.getElementById('btn-forward').onclick = () => motorAction('forward');
    document.getElementById('btn-stop').onclick = () => motorAction('stop');
    document.getElementById('btn-backward').onclick = () => motorAction('backward');
});
