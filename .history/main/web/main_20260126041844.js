document.addEventListener('DOMContentLoaded', () => {
    ...
    // UUIDs Updated to match iOS App and new ESP32 firmware
    ...
    async function updateDashboard() {
        try {
            ...
            if (resp.ok) {
                ...
                accDisplay.innerText = `X: ${data.acc[0].toFixed(1)} Y: ${data.acc[1].toFixed(1)} Z: ${data.acc[2].toFixed(1)}`;
                gpsDisplay.innerText = `${data.gps[0].toFixed(6)}, ${data.gps[1].toFixed(6)} (${data.sats}星)`;
            }
        } catch (e) {
            console.warn('Dashboard fetch failed (likely disconnected from AP)');
        }
    }
    ...
    shutdownBtn.addEventListener('click', async () => {
        if (confirm('确定要让系统断电并进入深度睡眠吗？(按 BOOT 键唤醒)')) {
            ...
            msgBar.innerText = '发送关机指令中...';
        }
    });

    // 修正 UUID 常量以防万一
    const SERVICE_UUID_FULL = '6e400001-b5a3-f393-e0a9-e50e24dcca9e';
    const CHAR_UUID_FULL    = '6e400002-b5a3-f393-e0a9-e50e24dcca9e';
});
